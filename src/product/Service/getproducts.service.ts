...
export class GetProductsService {
  ...
  async getProductById(productId: number) {
    ...
    return {
      ...
      ratingDistribution: this.getRatingDistribution((product as any).reviews ?? []),
    };
  }
  ...
  private getProductListSelect() {
    return {
      ...
      // Reviews and counts removed from schema — runtime checks used instead
    };
  }
  ...
  private getProductDetailSelect() {
    return {
      ...
      // Reviews and counts removed from schema — if needed fetch via separate endpoint
    };
  }
}