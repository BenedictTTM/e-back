...
export class SearchProductsService {
  ...
  private getProductSelect() {
    return {
      ...
      // reviews and counts removed from schema — rating data will be handled via separate aggregation if required
    };
  }
  ...
  private enrichProduct(product: any) {
    const averageRating = this.calculateAverageRating((product as any).reviews ?? []);
